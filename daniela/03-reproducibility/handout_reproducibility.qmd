...
```{r}
#| label: tbl-conda
#| tbl-cap: "conda and venv/pip commands are bash code to be entered in the shell (e.g., Terminal, PowerShell, Anaconda Prompt)"
...
::: {.callout-warning}
#### Avoid `conda` + `pip`
If you install packages via pip inside conda, `--from-history` won't capture them. Ideally install everything with conda install where possible, and use p`ip only for packages not on conda channels.
:::
...
3. Check the interpreter in your **status bar** -- it should show your conda environment name (e.g. `zas_python_wg`). If not: Command Palette -> Python: Select Interpreter -> choose it. Alternatively, check in Terminal with `which python` and `which python
python --version` (but you can't select it in the Terminal).
4. Open the integrated terminal (`Ctrl+\`` or Terminal -> New Terminal) -- it should activate your conda environment automatically
5. Verify with `conda info --envs` (asterisk = active env) and `pwd` (correct folder). If there is no asterisk (*) next to your environment, run `conda activate zas_python_wg` and verify.
:::
...
Open the `repro_exercises` exercises in whicher file format you prefer (`.py`, `.ipynb`, `.qmd`) and do the exercises under Part A 'git hygiene'.
