...
Every project README should answer three questions: what does this project do, how do I run it, and what do the outputs mean?
...
By default, Quarto re-runs all your code from scratch on every render -- correct for reproducibility, but slow for heavy computations like model fitting or preprocessing big datasets. `freeze: true` changes this: Quarto **caches each chunk's output** and only re-runs a chunk when its source code actually changes.
...
ex_a <- data.frame(
  Step = c("A1", "A2", "A3", "A4"),
  Task = c(
    "Activate your conda environment: conda activate zas_python_wg",
...
# -------------------------------------------------------
data_path = "/Users/replace-me/projects/zas_python_wg/data/raw/sample.csv"
