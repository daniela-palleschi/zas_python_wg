...
3. Check the interpreter in the **bottom status bar** -- it should show your conda environment name (e.g. `zas_python_wg`). If not: Command Palette -> Python: Select Interpreter -> choose it
...
`uv` is a package and project manager for Python written in Rust, by the team behind the `ruff` linter. It replaces venv, pip, pip-tools, and pyenv with a single tool and is 10-100x faster than pip for installs.
